...
<div class="max-w-2xl">
        ...
        <div class="bg-white rounded-xl border border-gray-100 p-6">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full px-4 py-2.5 border border-gray-200 text-sm focus:outline-none focus:border-[#d4941a] bg-white">
                            @foreach(['active', 'eliminated', 'qualified', 'winner'] as $s)
                ...
                <div>
                    ...
                    <select name="county_id" class="w-full px-4 py-2.5 border border-gray-200 text-sm focus:outline-none focus:border-[#d4941a] bg-white">
                        <option value="">No county assigned</option>
                        @foreach($regions as $region)
                            <optgroup label="{{ $region->name }}">
                                @foreach($region->counties as $county)
                                    <option value="{{ $county->id }}" {{ old('county_id', $contestant->county_id) == $county->id ? 'selected' : '' }}>
    ...
    {{-- Danger Zone (must be outside the main form — nested forms break HTML) --}}
